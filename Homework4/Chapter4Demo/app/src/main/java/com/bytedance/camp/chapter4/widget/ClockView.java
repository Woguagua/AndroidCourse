...
//        invalidate();
    ...
    private void drawTimeNeedles(Canvas canvas) {
        ...
        //时针
        double radians_hour = Math.toRadians((hour * 60 + minute) * 0.5 - 90);
        float startX = centerX;
        float startY = centerY;
        float stopX_hour = (float) (centerX + radius * HOUR_NEEDLE_LENGTH_RATIO * Math.cos(radians_hour));
        float stopY_hour = (float) (centerY + radius * HOUR_NEEDLE_LENGTH_RATIO * Math.sin(radians_hour));

        //分针
        double radians_min = Math.toRadians((minute * 60 + second) * 0.1 - 90);
        float stopX_min = (float) (centerX + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.cos(radians_min));
        float stopY_min = (float) (centerY + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.sin(radians_min));

        //秒针
        double radians_sec = Math.toRadians(second * 6 - 90);
        float stopX_sec = (float) (centerX + radius * SECOND_NEEDLE_LENGTH_RATIO * Math.cos(radians_sec));
        float stopY_sec = (float) (centerY + radius * SECOND_NEEDLE_LENGTH_RATIO * Math.sin(radians_sec));

        needlePaint.setAlpha(HIGHLIGHT_UNIT_ALPHA);
        needlePaint.setStrokeWidth(HOUR_NEEDLE_WIDTH);
        canvas.drawLine(startX,startY,stopX_hour,stopY_hour,needlePaint);//时针

        needlePaint.setAlpha(HIGHLIGHT_UNIT_ALPHA);
        needlePaint.setStrokeWidth(MINUTE_NEEDLE_WIDTH);
        canvas.drawLine(startX,startY,stopX_min,stopY_min,needlePaint);//分针

        needlePaint.setAlpha(NORMAL_UNIT_ALPHA);
        needlePaint.setStrokeWidth(SECOND_NEEDLE_WIDTH);
        canvas.drawLine(startX,startY,stopX_sec,stopY_sec,needlePaint);//秒针
